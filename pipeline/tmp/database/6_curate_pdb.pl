...
use File::Basename;
use Cwd 'abs_path';
my $bindir = dirname(dirname(abs_path(__FILE__))) . '/script';
...
foreach my $pdb(@pdb_list)
{
    ...
    next if ( -s "$outdir/$pdb.ignore" || !-s "$inputdir/$pdb.cif.gz" ||
            ( -s "$outdir/$pdb.txt"    && (-s "$outdir/$pdb.tar.gz"   ||
                                         -s "$outdir/$pdb.tar.bz2")));
