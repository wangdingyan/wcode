...
if __name__ == '__main__':
    PATH_11 = '/mnt/c/tmp/validPNEAR/A1/'
    DICT_11 = {}
    ...
    for n in DICT_11.keys():
        print(f"A1 {n}, Mean: {np.mean(DICT_11[n])} STD: {np.std(DICT_11[n])}, COUNT: {len(DICT_11[n])}")



    PATH_11 = '/mnt/c/tmp/validPNEAR/A2/'
    DICT_11 = {}
    names = os.listdir(PATH_11)
    for name in names:
        cls, num = name.split('_')
    ...
    for n in DICT_11.keys():
        print(f"A2 {n}, Mean: {np.mean(DICT_11[n])} STD: {np.std(DICT_11[n])}, COUNT: {len(DICT_11[n])}")
