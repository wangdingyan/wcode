...
class DefaultRunner(object):
    ...
    def evaluate(self, split, verbose=0):
        ...
        dataloader = DataLoader(test_set,
                                ...
                                collate_fn=dataset.CycPepDataset.collate_fn)
        model = self._model
        model.eval()
        # code here
        eval_start = time()
        ...
        for batch in dataloader:
            pad_seqs, pad_bonds, pad_angles, pad_dihedrals = batch
            if self.device.type == "cuda":
                pad_seqs, pad_bonds, pad_angles, pad_dihedrals = pad_seqs.cuda(), pad_bonds.cuda(), pad_angles.cuda(), pad_dihedrals.cuda()

            loss = model([pad_seqs, pad_bonds, pad_angles, pad_dihedrals], device=self.device)
            loss = loss.mean()
    ...
    def SDE_PC_sampeler(self,
        ...
        Args:
            ...
            SDE_model: trained CycGen model
        ...
        d_vecs = []
        pad_seqs, pad_bonds, pad_angles, pad_dihedrals = batch
        pad_dihedrals = pad_dihedrals.reshape(-1)

        num_edge = len(pad_dihedrals)
    ...
    def sde_generate_samples_from_mol(self,
                                      protein_sequence,
                                      num_repeat=1,
                                      out_path=None,
                                      file_name=None,
